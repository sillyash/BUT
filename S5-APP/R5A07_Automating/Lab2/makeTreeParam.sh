...
if [ -z "$1" ]; then
	echo "Error : No language sepcified as parameter. Extiting."
	exit 1
fi

# Input parameter assignation
lang="$1"
langDir="$lang-Data"

echo "Creating a directory tree for the corpus data in $lang"
...
mkdir -p bin
mkdir $langDir
chmod 755 bin $langDir

# Creating the README file
cd $langDir
echo "This is the README file from the multiword expressions (MWEs)" \
			"corpus for $lang, edition 1.4." > README.md
chmod 755 README.md
...
echo "Creating a symbolic link from $langDir/ to bin/."
ln -s ../bin scripts

# Creating a hard link
echo "Creating a hard link from $langDir/ to the global README.md."
